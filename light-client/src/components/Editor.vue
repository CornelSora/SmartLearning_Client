...
export default {
  ...
  mounted () {
    console.warn("https://35.178.185.118/")
    // if (this.$userID) {
  ...
  methods: {
    ...
    setContent () {
      debugger
      this.content = (!this.content || this.content.indexOf(this.defaultMesage) > -1 || this.content.indexOf('Enter your code here') > -1) ? this.setDefaultContent() : this.content
    },
    setDefaultContent() {
      if (!this.loadedAlone) {
        var localCode = this.$settings.getCode(this.language, this.problemID)
